...
async function load() {
  try {
    const [i,p,m] = await Promise.all([
      fetch('/api/info?token='+T).then(r=>r.ok?r.json():null).catch(()=>null),
      fetch('/api/players?token='+T).then(r=>r.ok?r.json():null).catch(()=>null),
      fetch('/api/metrics?token='+T).then(r=>r.ok?r.json():null).catch(()=>null)
    ]);
