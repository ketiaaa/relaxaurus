...
const H = savedToken ? { 'Authorization': 'Bearer ' + savedToken } : {};
if (!savedToken && !T) location.href = '/';
async function load() {
  try {
    const headers = { 'Authorization': 'Bearer ' + T };
    const [i,p,m] = await Promise.all([
      fetch('/api/info',{headers:H}).then(r=>r.ok?r.json():null).catch(()=>null),
      fetch('/api/players',{headers:H}).then(r=>r.ok?r.json():null).catch(()=>null),
      fetch('/api/metrics',{headers:H}).then(r=>r.ok?r.json():null).catch(()=>null)
    ]);
